  ...
  return (
    <div className="flex justify-center items-center gap-3 border-solid cursor-pointer box-border h-[90px] bg-[#FFFFFF] border-[2px] border-[#A52223] rounded-[50px]">
        <div  className="flex justify-center items-center gap-2 border-solid cursor-pointer box-border w-[300px] h-[60px] bg-[#FFFFFF] border-[2px] border-[#A52223] rounded-[50px]">
            <input id='findInput' onChange={handleFind} placeholder='Tìm kiếm...' className="font-quicksand font-normal text-[20px] leading-[25px] text-[#000000]"
                
            />

            <img src={magnifierIcon} onClick={handleFind} alt="" />
        </div>
            <img className={`border-solid border-2 rounded-full ${prop.asc ? 'border-[#28BBFF]' : 'border-gray-400'}`} src={increaseIcon} onClick={() => prop.setAsc(true)} alt="" />
            ...
            <select id="filterSort" onChange={handleTypeOfSort} className="w-[15%] cursor-pointer font-quicksand font-bold text-[20px] leading-[25px] text-center text-[#000000] flex-none flex-grow-0">
                <option className='w-[25px] h-[20px] font-quicksand font-normal text-[16px] leading-[20px] text-[#000000] box-border bg-[#C5C5C5] border border-black' value="sellPrice">Giá</option>
            ...
            <img className={`border-solid border-2 rounded-full ${!prop.asc ? 'border-[#28BBFF]' : 'border-gray-400'}`} src={decreaseIcon} onClick={() => prop.setAsc(false)} alt="" />
    </div>
  )
}
